...
const Lousa = ({ htmlContent, toggleCanva }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative flex w-full h-full flex-col rounded-lg bg-white shadow-2xl dark:bg-darkBg-primary">
        <div className="flex items-center justify-between rounded-t-lg bg-lightBg-secondary p-2 dark:bg-darkBg-secondary">
          ...
          <Button variant="danger" size="icon" $rounded onClick={() => toggleCanva()}><LuX size={16} /></Button>
        </div>
        <iframe srcDoc={htmlContent} title="Lousa" sandbox="allow-scripts allow-same-origin" className="h-full w-full flex-1 rounded-b-lg border-none" />
      </div>
    </div>
  )
}
