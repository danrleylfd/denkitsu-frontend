...
const createLinker = async (label, link) => {
  try {
    ...
    console.error("Error creating linker:", error.response?.data || error.message)
    throw error
  }
}
...
const getLinkersByUser = async () => {
  try {
    ...
    console.error("Error fetching user linkers:", error.response?.data || error.message)
...
const updateLinker = async (oldLabel, newLabel, newLink) => {
  try {
    ...
    console.error(`Error updating linker ${oldLabel}:`, error.response?.data || error.message)
    throw error
  }
}
...
const deleteLinker = async (label) => {
  try {
    ...
    console.error(`Error deleting linker ${label}:`, error.response?.data || error.message)
    throw error
  }
}
...
const getLinkByLabel = async (label) => {
  try {
    ...
    console.error(`Error fetching link for label ${label}:`, error.response?.data || error.message)
    throw error
  }
}
