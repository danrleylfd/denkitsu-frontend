...
import { LuFileCode2, LuUpload, LuCopy, LuDownload, LuArrowLeft } from 'react-icons/lu';

import SideMenu from '../components/SideMenu'

// --- Funções de Lógica (fora do componente para não serem recriadas) ---

// Função semelhante a 'fnmatch' para padrões glob simples, usada pela lógica do .gitignore
const fnmatch = (pattern, string) => {
    // Esta é uma versão simplificada de fnmatch, não cobre todos os casos extremos,
    // mas abrange padrões comuns do .gitignore como *, ?, e caminhos básicos.
    // Converte um padrão glob em uma regex.
    const regex = new RegExp(
        '^' +
        pattern
            .replace(/([.$+^|(){}\[\]\\])/g, '\\$1') // Escapa caracteres especiais de regex
            .replace(/\*/g, '.*') // '*' corresponde a qualquer sequência de caracteres
            .replace(/\?/g, '.') + // '?' corresponde a qualquer caractere único
        '$'
    );
...
const shouldIgnore = (relativePath, options) => {
    ...
    // Regra: Padrões do .gitignore têm a maior prioridade
    if (options.use_gitignore && options.gitignore_patterns) {
        let isIgnored = null; // null = não correspondido, true = ignorado, false = não ignorado
        for (const pattern of options.gitignore_patterns) {
            ...
            if (currentPattern.endsWith('/')) {
                // Padrões terminados com '/' correspondem a diretórios em qualquer lugar no caminho
                const dirPattern = currentPattern.slice(0, -1);
                if (pathParts.includes(dirPattern)) {
                    match = true;
                }
            } else if (!currentPattern.includes('/')) {
                // Padrões sem '/' correspondem a nomes de arquivos ou diretórios em qualquer nível
                if (pathParts.some(part => fnmatch(currentPattern, part))) {
                    match = true;
                }
            } else {
                // Padrões com '/' são comparados com o caminho relativo completo a partir da raiz
    ...
    // Regra: Pastas de dependências padrão como node_modules
    ...
    // Regra: Outras opções específicas
    ...
    const defaultIgnoreExts = new Set(['.pyc', '.pyo', '.pyd', '.so', '.o', '.a', '.dll', '.exe', '.tmp', '.swp', '.swo', '.user', '.suo', '.pdb', '.DS_Store', '.class', '.jar', '.war', '.ear', '.png', '.jpg', 'jpeg', '.gif', '.bmp', '.ico', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.zip', '.tar', '.gz', '.rar', '.7z', '.mp3', '.mp4', '.mov', '.avi']);
...
const OptionsPanel = ({ options, setOptions }) => {
    ...
    // Mapeamento de chaves de opções para textos em português
    const ptLabels = {
        ...
        <aside className="glass-panel">
            ...
            <div className="space-y-3 text-sm">
                {Object.keys(options).map(key => {
                    ...
                    return (
                        <label key={key} className={`flex items-center space-x-3 cursor-pointer ${key === 'keep_readme' ? 'ml-4' : ''} ${isDisabled ? 'text-slate-400 cursor-not-allowed' : 'text-slate-200 hover:text-white'}`}>
                ...
                <div>
                    <label htmlFor="custom_patterns" className="text-sm font-medium text-slate-200">Regras personalizadas (separadas por vírgula):</label>
                    <input
                        type="text"
                        id="custom_patterns"
                        value={options.custom_patterns.join(', ')}
                        onChange={(e) => setOptions(prev => ({ ...prev, custom_patterns: e.target.value.split(',').map(p => p.trim()).filter(p => p) }))}
    ...
    return (
        ...
        >
            <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-all duration-300 ${isDragging ? 'bg-blue-500/30' : 'bg-white/10'}`}>
                <LuUpload className={`w-10 h-10 transition-transform ${isDragging ? 'text-blue-300 scale-110' : 'text-slate-300'}`} />
            </div>
...
const ResultView = ({ result, onReset }) => {
    ...
    return (
        <div className="glass-panel h-full overflow-hidden flex flex-col p-0">
            ...
            <textarea value={result} readOnly className="w-full flex-grow p-4 font-mono text-sm bg-slate-900/70 text-slate-200 border-none focus:ring-0 resize-none"></textarea>
        </div>
    );
};
...
export default function App() {
    ...
    const [options, setOptions] = useState({
        use_gitignore: true, ignore_dot_folders: true, ignore_config_dot_files: true,
        ignore_env_files: true, ignore_vendor_folders: true, ignore_build_folders: true,
        ignore_license: true, ignore_logs: true, ignore_lock_files: true,
        ignore_markdown: true, keep_readme: true, remove_blank_lines: true,
        custom_patterns: [],
    });

    const processFiles = useCallback((files, rootName) => {
        ...
        try {
            ...
            if (options.use_gitignore && gitignoreFile) {
                gitignorePatterns = gitignoreFile.content.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'));
            }

            setStatusText('A gerar codebase...');
            const currentOptions = { ...options, gitignore_patterns: gitignorePatterns };

            const includedFiles = files.filter(({ relativePath, content }) =>
                !isBinaryContent(content) && !shouldIgnore(relativePath, currentOptions)
            );
            ...
            const fileTree = generateFileTree(includedFiles.map(f => f.relativePath), rootName);
    ...
    const handleDrop = useCallback(async (items) => {
        ...
        try {
            ...
            const traverse = async (entry, currentPath) => {
                if (entry.isFile) {
                    filePromises.push(new Promise(resolve => entry.file(async file => {
                        try {
    ...
    const handleReset = () => { setView('drop'); setResult(''); };
