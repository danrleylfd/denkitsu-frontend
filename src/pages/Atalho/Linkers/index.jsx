...
import { SideContentContainer, FormControl, LinkItem, LinkDetails, LinkActions } from "./styles"
...
const Linkers = () => {
  ...
  const handleDeleteLink = async (labelToDelete) => {
    if (!window.confirm(`Tem certeza que deseja excluir o link com rótulo "${labelToDelete}"?`)) {
      return
    }
    try {
  ...
  return (
    <SideMenu style={{ position: "fixed" }} ContentView={SideContentContainer}>
      ...
      <Paper>
        <FormControl onSubmit={handleCreateLink}>
      ...
      {!loading && linkers.length > 0 && (
        <>
          {linkers.map((linker) => (
            <LinkItem key={linker._id}>
              {editingLinker?._id === linker._id ? (
                <FormControl onSubmit={handleUpdateLink} style={{ width: "100%" }}>
                  ...
                  {editError && <MessageError style={{ textAlign: "left", marginTop: "5px" }}>{editError}</MessageError>}
                </FormControl>
              ) : (
                <>
                  <LinkDetails>
