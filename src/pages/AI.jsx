...
  }, [notifyError, aiContext, selectedPrompt])
  ...
  // MODIFICADO: A função agora aceita um parâmetro 'forceSend'.
  const onSendMessage = useCallback(async (forceSend = false) => {
    // MODIFICADO: A verificação de bloqueio agora inclui 'forceSend'. Se for true, a função continua mesmo com o input de texto vazio.
    if (loading || (!aiContext.userPrompt.trim() && aiContext.imageUrls.length === 0 && !forceSend)) return
   
    // MODIFICADO: Lógica para construir o conteúdo da mensagem do usuário.
    const promptContent = aiContext.userPrompt.trim()
    const newContent = []
    if (promptContent) {
        newContent.push({ type: "text", content: promptContent })
    }
    if (aiContext.imageUrls.length > 0) {
        newContent.push(...aiContext.imageUrls.map(url => ({ type: "image_url", image_url: { url } })))
    }
...
        <AITools loading={loading} toolsDoor={toolsDoor} />
        {/* MODIFICADO: A função 'onSendMessage' atualizada é passada para a barra de chat. */}
        <AIBar
          loading={loading}
